...
class formChartDouble extends StatelessWidget {
  ...
  List valueTypes = [];
  formChartDouble({Key key, @required this.PlotKey, this.type, this.color}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text(CATALOG_NAMES[CATALOG_TYPES[type]] + " of " + PlotKey.Name),
  ...
  List<Widget> _createTabs(context){
    ...
    if(DataElements.length>0) {
      ...
      for (var index = 0; index < DataElements.length; index++) {
        String date = _getDate(DataElements[index].timestamp);
    ...
    for(int i = 0; i<dias.length;i++){
        fin.add(createGraph(context, dias[dias.keys.toList()[i]],dias.length-(i+1)));
    }
        ...
        child: Padding(
          ...
          child: GestureDetector(
            onTap: (){
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => formVisualization(PlotKey: PlotKey,)),
              );
            },
            child: Card(
